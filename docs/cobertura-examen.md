...
**2026-09-24, sesión de TLA+**: `EX-07` se cierra (rama `tla-backend`). Queda bloqueante
**`EX-04`**.
...
| Spec | Para | Estado |
| --- | --- | --- |
| `SPEC-27` | Exportar a PDF, y corregir la fila de `SPEC-22` §1.2 (`EX-09`) | `aplicada` el 2026-09-24: `PLAN-27` E1–E10, de `4ea4b6b` a `a991278`, con `ejemplos/novela-ejemplo.pdf` (68 páginas) de una novela que pasó la puerta |
...
| **EX-06** | § "Evaluación del sistema" `:92`–`:94`: cinco briefs (uno de injection, uno temporal), tabla por brief, una iteración de tuning con antes y después; § `/docs`: red-team log | `harness/evals/README.md` tiene **uno de cinco** (`brief-incoherencia-temporal.json`, sin ejecutar). `harness/adversarial/` está declarada en `docs/architecture.md` § "El harness" y no existe; `VER-30` está obsoleta. `SPEC-26` lo deja fuera (`:183`) | Ausencia | **Documentado en vuelta 2**: lo recoge `SPEC-31`, `aprobada` el 2026-09-23. Deja de ser hueco de documentación y **pasa a hueco de sistema**: va a su plan **Vuelta de `PLAN-31`**: los cinco briefs existen y cargan (`VER-120`), la tabla se genera en `harness/evals/resultados.md` y hoy dice «sin ejecutar» en todo (`VER-121`), el red-team vive en `harness/adversarial/` (`VER-126`) y encontró una fuga entre novelas (`F-100`). **Hueco de sistema abierto**: no hay ninguna ejecución, ni el tuning. **Vuelta del 2026-09-25:** la pasada «antes» está **ejecutada con los cinco briefs y el modelo real** (`R1` a `R5`, 47,1353 USD sumando la tabla de `resultados.md`): dos novelas publicadas (base y vetadas), la inyección contenida en la entrada, la incoherencia temporal parada por el Revisor en el plan y las contradicciones detectadas por el Entrevistador. El red-team contra el modelo real está en `docs/proceso/red-team-log.md`. **Sigue abierto**: la revisión humana (`H1`). **El tuning queda no completado** por decisión del autor (2026-09-25): `T1` commiteado sin medir; la «antes» repetida del brief base se paró tres veces en el capítulo 3 por `INV-03` (8,7712 USD) y la «después» no se lanzó. La causa medida es `F-155`: el Planificador no siembra nunca el `conocimiento_inicial`. Detalle en `docs/proceso/registro-de-iteraciones.md` |
| **EX-07** | §5d `:87` *«La especificación debe corresponder al código: el README explica qué estado o transición del código implementa cada acción»* | `specs/tla/README.md` § "Qué implementa cada acción" (`:83`) modela *«el flujo por capítulos de `main`»* y cita `src/config.py`, `src/servidor.py` y `EJECUCION.md`, que **no están en este árbol**. El pipeline de la novela regalo vive en `backend/` | Contradicción | **Cerrado (2026-09-24), rama `tla-backend`.** `specs/tla/HarnessBackend.tla` modela `backend/` y la tabla de `specs/tla/README.md` da, por acción, la función y las líneas que la implementan, y si existe hoy o espera a un paso de `PLAN-23`. La escalera de modelos desapareció con `main`. TLC pasa el modelo corregido y rompe nueve invariantes con el código de hoy: siete contraejemplos de `backend/` (`F-110`…`F-114`, `F-116` en `docs/verification.md`) y dos del diseño de `PLAN-23`. Lo que queda abierto no es de este hueco sino de esos hallazgos: **ninguno está arreglado en el código**, y la vivacidad del código de hoy está comprobada en tres capítulos, no en cinco |
...
| **EX-15** | §5c `:73` un caso **real** en que Lean detecta lo que los otros no | `specs/lean/README.md` § "El caso real" ejecuta Lean sobre *«una base… sembrada con una obra de cuatro eventos»* | **Cerrado el 2026-09-25 con una generación real**: `obra-9df1eb4deb`, diez capítulos consolidados, la puerta no la publicó por 52 violaciones de Lean (8 `L-1`, 44 `L-3`) sin ningún hallazgo de otro validador; salida literal en `specs/lean/casos/obra-9df1eb4deb/` (`F-213`, `F-214`). Antes: nada fallaba, faltaba una generación. `L-1` lo caza también `INV-08` desde `b2f097c`. `L-2` sigue siendo solo de Lean (`edades()` no tiene llamadas fuera de sus pruebas). `L-3` solo en parte: `INV-02` cubre la accesibilidad escena a escena, y lo que queda solo en Lean es la comparación entre escenas del mismo momento (`specs/lean/README.md`). **Corregido en la vuelta 3**: la vuelta 1 decía que `L-3` era solo de Lean | Ejecutar `harness/evals/brief-incoherencia-temporal.json` cuando exista `EX-03` |
