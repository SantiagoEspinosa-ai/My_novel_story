...
| **VER-118** | La rama que elija la medida | T | — | **Reservada para la Parte B de `PLAN-23`**, sin escribir: se decide con el número de B1 | El de la rama, que se escribe en B3 | — |
...
| F-146 | **Las sesiones delegadas de Claude Code anonimizan los nombres de la ficha, por una política de la organización que está fuera del repositorio, y eso rompe la personalización.** En el brief de contradicciones (`R4`) el Entrevistador guardó al destinatario como `[NOMBRE_ANONIMIZADO]` en la ficha y el Planificador lo copió en las tres versiones del plan; la cobertura rechazó las tres (*«el personaje «[NOMBRE_ANONIMIZADO]» parece «[NOMBRE_ANONIMIZADO]» mal escrito»*) y no hubo novela. Aparece en `entrevista`, `turno_de_entrevista`, `plan_de_obra` y `trabajo`. **No es estable**: en la novela de ejemplo y en la de vetadas, cuya ficha no pasó por el Entrevistador, los nombres llegaron intactos. Y el problema es de fondo: en una novela de verdad los nombres **son** de personas reales, que es justo lo que la política protege | **Abierto, y lo decide el autor**: es una tensión entre el producto (personalizar con el nombre real) y una política de la organización. No se sortea diciéndole al modelo que la ignore |
