...
| F-111 | **Relanzar la planificación pisa las filas de `plan_de_obra` de la ejecución anterior** (Regla 7). `planificar` vuelve a numerar desde `version = 0` (`planificacion/service.py:130-132`) y `repository.guardar` escribe con `INSERT OR REPLACE` sobre la clave `(obra, version)` (`planificacion/repository.py:24`, `:36`). El camino es el que el propio informe recomienda: el Planificador o el Revisor agotan sus reintentos de transporte (`ConReintentos`, `novela.py:302-304`), `FalloDeTransporte` sube hasta `novela_regalo.main`, que imprime *«relanzar reanuda»* (`novela_regalo.py:193-208`); al relanzar no hay plan aprobado y la ronda 1 nueva **sustituye** a la ronda 1 vieja. Lo que se pierde son las objeciones de las rondas anteriores, que es lo primero que `F-69` y `SPEC-26` `RF-07` quieren poder leer cuando un plan no se aprueba. No falla nada. TLC: `CE-8`, invariante `RondasDePlanConservadas` | **Abierto.** Comparte arreglo con `F-110`: la `version` siguiente sale de la base, no de un contador local |
