...
| F-61 | **Los hooks no dejaron constancia en la ejecución real.** El Escritor completó tres sesiones y el registro de `HARNESS_REGISTRO_HOOKS` no se creó. Puede ser que Claude Code no cargue los hooks de `.claude/settings.json` en una sesión `-p` lanzada desde `backend/`, que el comando `python "$CLAUDE_PROJECT_DIR/..."` no funcione en la shell de Windows, o que la variable no llegue al proceso del hook. **La causa está sin diagnosticar**; lo único medido es la ausencia. Es exactamente lo que `VER-76` declaraba como su punto ciego | **Abierto, con la causa diagnosticada.** Dos sesiones mínimas iguales (`claude -p`, modelo pequeño, `HARNESS_AGENTE` y `HARNESS_REGISTRO_HOOKS`): lanzada desde `backend/` **no deja registro**; lanzada desde la raíz del repositorio, **sí** (`validar_capitulo` sobre `resumidor`, código 0). Claude Code carga los hooks de `.claude/settings.json` solo si la sesión arranca en la carpeta que lo contiene, y `SesionDelegada` hereda el directorio del proceso que la lanza, que en los guiones es `backend/`. Coste del diagnóstico: 0,0919 + 0,0510 USD, medidos. El arreglo propuesto —que las delegaciones arranquen en la raíz del repositorio— espera a que el usuario vea el diagnóstico |
