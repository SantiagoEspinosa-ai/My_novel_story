...
| F-64 | **Dos novelas en la misma base se pisan: los identificadores de escena, entidad y evento son globales, y la novela regalo los repite en todas las obras.** `escena.id`, `entidad.id` y el `id` de `evento_cronologico` son claves primarias sin obra; `montar` numera las escenas `"{capitulo}-e1"` y los imprescindibles `imp-01`, `imp-02`… en todas las novelas; y `uso_de_hecho` no guarda la obra. **Y no falla: pisa.** `hecho_canonico.id` también es global y `declarar_hechos` hace `INSERT OR REPLACE`, así que la segunda novela sobrescribe en silencio los imprescindibles de la primera (Regla 7). Tampoco es improbable: `novela_regalo.py` usa `backend/regalo.db` por defecto, y dos novelas lanzadas sin `--base` comparten base. Es `F-39` con otra cara: allí se pisaban hechos entre capítulos, aquí obras enteras. La parte de la novela regalo la confirmó la otra sesión, que la escribió. Lo encontró la preparación de `PLAN-28`, y bloquea las ejecuciones reales de `PLAN-31`, que comparten una base | **Abierto.** Es un defecto activo, no un hueco: no decide nada nuevo y se arregla antes de las ejecuciones reales de `PLAN-31`. La prueba que lo reproduce es la de exfiltración de `PLAN-31` E7 |
