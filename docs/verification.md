...
| F-68 | **Los errores de formato del Planificador gastan las rondas de revisión, y el plan no se aprueba.** Segunda ejecución real (`PLAN-26` E13, ficha real): ronda 1 rechazada por el esquema (el POV de un capítulo apunta a un personaje no declarado), ronda 2 rechazada por el esquema (un campo inventado, `personajes_nota`, pese al aviso de `F-62`) y ronda 3 rechazada por el Revisor con tres objeciones de fondo. Con el tope de 3 agotado, `PlanNoAprobado` y ningún capítulo. **El Revisor solo llegó a ver un plan**: dos de las tres rondas se fueron en fallos que el código detecta sin juicio. `F-62` (el aviso en el prompt) no bastó | **Abierto, con el arreglo decidido.** El autor aprueba la propuesta (2026-09-24): un plan fuera de esquema no cuenta como ronda de revisión y se reintenta con su propio tope, como un fallo de transporte. Pendiente de código |
