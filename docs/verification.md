...
| F-72 | **El reintento por fallo de transporte está declarado y no existe.** `reintentos_de_transporte` (`commons/configuracion/esquemas.py:357`, 3 en `backend/config/sistema.json`) no lo lee ningún código, y `se_reintenta` y `quedan_intentos` (`commons/modelo/cliente.py:19`, `:25`) no tienen llamadas fuera de sus pruebas. En `orquestacion/obra.py` una delegación con cualquier `fallo` corta el bucle. El enunciado §3 pide *«Retries con límite»*, y el tope está en la configuración sin que nadie lo aplique. Lo encontró la auditoría del 2026-09-24 | **Abierto.** Defecto activo, sin decisión nueva: el tope ya está elegido |
