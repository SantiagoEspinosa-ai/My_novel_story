...
| GastoDeDelegacion | Lo que costó una delegación, sea del ciclo de escena o de fuera (Planificador, Revisor, juicio de la puerta, Entrevistador), guardado en la base (`SPEC-33` `RF-18`). Hasta la migración 18 el coste solo llegaba a Langfuse y a la memoria (`F-144`), así que lo gastado de una base es un **suelo**. | **obra** → Obra, **agente**, generacion (el identificador de la ejecución; ausente fuera de una generación, como en un turno de entrevista), coste\_usd (**ausente si no se midió, nunca cero**), **cuando** |
...
| TurnoDeEntrevista | Un turno de la conversación con el comprador: su respuesta, la pregunta que vino después y lo que el código dijo en ese turno. Es lo que la web usa para reconstruir la conversación al recargar (`SPEC-33` `RF-10`). Lleva datos personales: **se borra con la ficha** (`SPEC-25` `RF-21`). La primera pregunta no pertenece a ningún turno. | **entrevista** (el id de la entrevista), **orden** (1-based), **respuesta**, **pregunta** (la siguiente, no la contestada), tema, falta\[\], avisos\[\], contradicciones\_abiertas\[\] (`{tipo, descripcion}`: las que seguían abiertas tras el turno), cuando, fuera\_del\_modelo (el turno se contestó en el campo de nombres y no llegó a ningún agente: `SPEC-34` `RF-01`; ausente en los turnos anteriores a la migración 19). En un turno anterior a la migración 17, `falta`, `avisos` y `contradicciones_abiertas` están **ausentes**: no se guardaron, que no es lo mismo que una lista vacía |
| Pseudonimo | Una palabra de un nombre real de la ficha y el nombre inventado que la sustituye ante los agentes, en una obra (`SPEC-34` `RF-02`, `RF-08`). Es una pareja guardada, no un cálculo: se elige una vez y se lee siempre. Ninguna palabra real ni ningún pseudónimo de la obra comparte raíz con otro, y ninguno choca con una vetada ni con un nombre vetado. Lleva datos personales: **se borra con la ficha** (`SPEC-25` `RF-21`) | **obra**, **palabra\_real**, **pseudonimo**, **titular** → `titular_de_pseudonimo` |
...
| sustituye\_ante\_los\_agentes | Pseudonimo | Obra | N:1 (una pareja por palabra real y obra) | La frontera de los nombres (`SPEC-34`): el prompt sale con el pseudónimo y la respuesta vuelve con la palabra real |
...
| vigente | Obra | El **numero** de la `VersionDeObra` que ve el lector: la última que la puerta de publicación publicó o, si no hay ninguna, la 1. No es la última creada: la cascada crea la versión antes de escribirla (`F-121`, TLC `CE-14`). La calcula `commons/obra/vigente.py`, en un solo sitio, y la API la dice; la web no la deduce (`F-150`) | `PLAN-23` `F-121`, `F-150` |
| compartido | Capitulo (en una VersionDeObra) | Verdadero si el capítulo de esa posición es **el mismo** —la misma fila— que el de la misma posición en la versión `anterior`; falso si cambió; **nulo** si la versión no tiene anterior. Por identidad, nunca comparando textos (`CE-5`): un capítulo nuevo con el mismo texto cuenta como cambiado. Es la marca de «cambió» de `RF-52`; la calcula `regeneracion` y la lectura de una versión solo la lleva | `PLAN-23` A5, `PLAN-22` E15 |
| cuaderno | FichaDeEntrevista | Lo que la ficha ya sabe (cada campo obligatorio de `que_falta` con su etiqueta y sus valores **en palabras de persona**: «cumpleaños», no `cumpleanos`; un campo en `otro`, con las palabras del comprador), lo que falta, en el orden de `que_falta`, cuántos son en total y cuántos faltan, y la propuesta de **titulo**, **premisa** y **dedicatoria** (`SPEC-35` `RF-05`, `RF-07`) | `features/entrevista/cuaderno.py`, en el historial de la entrevista |
| historia | Obra (en la administración) | La historia de la novela en orden —entrevista, rondas del plan, capítulos con sus seis notas, intentos y coste, paradas, rondas de la puerta con Lean y versiones con su petición y sus capítulos cambiados— y sus totales: coste por agente, nota media, paradas y hallazgos abiertos. **El coste por capítulo es una atribución**: cada gasto a la última fila de `ProgresoDeGeneracion` anterior a él (`SPEC-37`) | `features/regalo/historia.py`, en `GET /admin/obras/{id}/historia` |
| fase\_de\_la\_obra, motivo\_del\_fallo | Obra (en su generación) | La última fase del progreso de la obra, y el motivo del último lanzamiento desde la web si terminó fallido o abandonado: lo que dice la página de la generación aunque no haya empezado ningún capítulo (`SPEC-35` `RF-13`, `F-206`) | `features/regalo/`, en la generación en vivo |
...
| `titular_de_pseudonimo` | Pseudonimo.titular | destinatario, quien\_regala, persona, mascota |
...
| INV-02 | Todo personaje presente tiene `estado_vital = vivo` y es accesible en `EstadoDelMundo(t)`. El estado vital cuenta antes **o** después de los cambios de la propia escena: un desaparecido que el delta devuelve a `vivo` y un vivo que desaparece o muere en ella pueden estar presentes (decisión del autor, 2026-09-24, completada en `F-148`; familia de `SPEC-16`). La accesibilidad es de vecino directo por `Lugar.accesos_y_salidas`. Punto ciego: las puertas leen el delta, no la prosa | escena | bloqueante | regla | `Escena.personajes_presentes`, `EstadoDelMundo.entidades_vivas`, `EstadoDelMundo.ubicaciones`, `Lugar.accesos_y_salidas` |
...
| INV-31 | Ningún texto restituido contiene un pseudónimo ni una forma derivada de él (un diminutivo o un plural que no se pudo restituir exactamente: `SPEC-34` `RF-04`) | escena | mayor | regla | `Borrador.texto`, `Pseudonimo` |
