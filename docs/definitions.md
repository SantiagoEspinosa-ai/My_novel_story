...
| GastoDeDelegacion | Lo que costó una delegación, sea del ciclo de escena o de fuera (Planificador, Revisor, juicio de la puerta, Entrevistador), guardado en la base (`SPEC-33` `RF-18`). Hasta la migración 17 el coste solo llegaba a Langfuse y a la memoria (`F-144`), así que lo gastado de una base es un **suelo**. | **obra** → Obra, **agente**, generacion (el identificador de la ejecución; ausente fuera de una generación, como en un turno de entrevista), coste\_usd (**ausente si no se midió, nunca cero**), **cuando** |
