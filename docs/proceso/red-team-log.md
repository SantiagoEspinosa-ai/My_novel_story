...
| **Un agente que intenta usar una herramienta** | `test_policy_niega_la_herramienta_y_lo_deja_en_el_audit_log` | El hook `policy.py` | Denegada y registrada | Ver abajo: en la ejecución real el hook **no se cargó** |
...
| **Los hooks no se cargaron** (`F-61`) | La primera ejecución real | Nadie en el momento: no dejaron constancia | Diagnosticado —Claude Code solo los carga desde la raíz del repositorio—, **sin resolver**. Mientras siga así, en una ejecución real ni el hook de validación ni el de policy actúan; las puertas del código sí |
...
| Exfiltración de datos entre dos novelas, a propósito | Sin ejecutar. Las tools solo leerán su obra (`SPEC-28` `RF-06`) |
