...
def obraReal : Obra :=
  ...
  [
    ...
    { id := "ev-3b", tFabula := { anio := 2019, mes := 6, dia := 12, hora := 18, minuto := 0 }, inicioMin := 62822520, duracionMin := 60, lugar := "lug-bosque", tDiscurso := 4, capitulo := "cap-3", analepsis := false, excluye := ["per-luis"], participan := [{ personaje := "per-luis", presencia := Presencia.presente }, { personaje := "per-marta", presencia := Presencia.presente }] },
    { id := "ev-4", tFabula := { anio := 2019, mes := 6, dia := 20, hora := 11, minuto := 0 }, inicioMin := 62833620, duracionMin := 30, lugar := "lug-casa", tDiscurso := 5, capitulo := "cap-4", analepsis := false, excluye := [], participan := [{ personaje := "per-luis", presencia := Presencia.presente }] }
  ]
  }
