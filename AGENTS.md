...
| Plan de verificación del sistema | `Docs/verification.md` | Primero **qué puede salir mal**: 23 modos de fallo `MF-01`…`MF-23` sobre las rejillas de MAST, ConStory-Bench y los fallos silenciosos, **ninguno sin estado y solo uno sin validador**. Después **cómo se detecta**: 54 validadores `VER-01`…`VER-55`, cada uno con su punto ciego, más los 10 asumidos y lo que se aprendió al escribir cinco. Ninguno implementado hoy |
| Skills del proyecto | `.agents/skills/` | Contenido real de las siete skills instaladas. Ver la sección "Skills" más abajo |
| Specs | `specs/` | Un fichero por spec, sin carpetas anidadas. Hoy: `SPEC - Backend.md` = `SPEC-01` (`en_revision`) y `SPEC - Referencias del dominio.md` = `SPEC-03` (`aplicada`). `SPEC-02`, vocabularios, se aplicó y su fichero se retiró |
