...
| Specs en curso | `specs/` | Un fichero por spec. **`aprobada`**: `SPEC-01` backend del harness, `SPEC-22` frontend y contrato congelado, `SPEC-21` usos de un hecho y cronología de la fábula, y `SPEC-24` dónde se declara una analepsis (pendiente de plan). **`en_revision`**: `SPEC-09` versión de `Resumen`, `SPEC-23` qué es regenerar en una obra acumulativa, y las cuatro que salen de `EXAMEN.md` (`docs/cobertura-examen.md`): `SPEC-27` exportar a PDF, `SPEC-29` Langfuse y qué sube y qué no, `SPEC-30` la puerta de publicación con Lean y `SPEC-31` la evaluación. `SPEC-28`, las tools, está reservada y sin redactar |
