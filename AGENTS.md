...
| Skills y herramientas | `myFactory/` | `skills/`: las ocho skills, un `PROCEDENCIA.md` en cada una (repositorio, commit y fecha) y en su `README.md` cuándo usar cada una. `repos-gigantes/`: herramientas para repositorios de millones de líneas, con inventario (`informe-por-categoria.md`) y decisión (`RECOMENDACION.md`) |
| Specs en curso | `specs/` | Un fichero por spec. **`aprobada`**: `SPEC-01` backend del harness, `SPEC-21` usos de un hecho y cronología de la fábula, `SPEC-22` frontend y contrato congelado, `SPEC-24` dónde se declara una analepsis, y las tres que salen de `EXAMEN.md` (`docs/cobertura-examen.md`): `SPEC-27` exportar a PDF, `SPEC-29` Langfuse y qué sube y qué no y `SPEC-31` la evaluación. **`en_revision`**: `SPEC-09` versión de `Resumen` y `SPEC-23` qué es regenerar en una obra acumulativa |
...
Una rama solo puede estar checkouteada en **un** worktree a la vez. Lo ignorado —`.env`, `*.db`, `salida/`— no viaja a un worktree nuevo y hay que copiarlo a mano; las skills sí, porque `myFactory/skills/` está versionado.
