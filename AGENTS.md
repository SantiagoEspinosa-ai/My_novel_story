...
| Briefs de evaluación | `harness/evals/` | Los cinco briefs de `SPEC-31` (base, injection, temporal, contradicciones y vetadas por variantes), cada uno con una ficha o un guion de entrevista, y `resultados.md`, la tabla por brief que genera `backend/evaluar.py`. **Ninguno ejecutado todavía** contra el modelo |
| Red-team | `harness/adversarial/` | `casos.json` (`RT-01`…`RT-06`: cada caso con su detector o `ninguno`) y las dos fichas disjuntas del caso de exfiltración. Los casos anteriores siguen en `docs/proceso/red-team-log.md`, que enlaza aquí |
...
| Foto del cierre | `docs/estado-al-cerrar-backend.md` | Dónde se quedó cada rama el 2026-09-25 (fusionado, sin fusionar, sin subir a GitHub), cómo levantarlo todo desde un clon con lo que no viaja por git, y qué pantallas de la web se abren de verdad. Foto fechada, no normativa |
...
| Specs en curso | `specs/` | Un fichero por spec. **`aprobada`**: `SPEC-01` backend del harness, `SPEC-21` usos de un hecho y cronología de la fábula, `SPEC-22` frontend y contrato congelado, `SPEC-23` qué es regenerar en una obra acumulativa (con la salida decidida por el arrastre medido), `SPEC-24` dónde se declara una analepsis, y las tres que salen de `EXAMEN.md` (`docs/cobertura-examen.md`): `SPEC-29` Langfuse y qué sube y qué no y `SPEC-31` la evaluación, `SPEC-33` la novela regalo en la web (estantería, entrevista, generación en vivo y coste). **`en_revision`**: `SPEC-09` versión de `Resumen` |
| Planes de implementación | `specs/plans/` | Un `PLAN-NN.md` por spec aprobada, con el mismo identificador y su propio estado. Hoy `PLAN-01`, el backend, en `en_revision`, `PLAN-22`, la lectura web y el contrato congelado, y `PLAN-23`, la regeneración, los dos **`aprobada`**, `PLAN-21`, los usos y la cronología, **`aprobada`**, `PLAN-25`, el destinatario y la entrevista, **`aplicada`**, `PLAN-26`, el pipeline de la novela regalo, **`aplicada`**, y los seis que salen de `EXAMEN.md`: **`aprobada`** `PLAN-29` y `PLAN-31`, y **`aplicada`** `PLAN-27`, `PLAN-28`, `PLAN-30` y `PLAN-32`; `PLAN-34`, los nombres fuera del modelo, **`aplicada`**, y `PLAN-33`, la novela regalo en la web, **`aprobada`**, `PLAN-35`, las seis pantallas rediseñadas (las dos tandas), **`aplicada`**, `PLAN-36`, la estantería de madera, la mesa y la administración, **`aplicada`**, `PLAN-37`, la historia de cada novela, **`aplicada`**, `PLAN-38`, la matriz por capítulo, **`aplicada`**, `PLAN-39`, publicar y reanudar, **`aplicada`**, `PLAN-40`, el protagonista ante los agentes, **`aplicada`**, `PLAN-41`, retirar y el techo entre capítulos, **`aplicada`**, `PLAN-42`, la administración enseña la estantería, **`aplicada`**, `PLAN-43`, la lectura para el lector, **`aplicada`**, `PLAN-44`, generar o reanudar desde la administración, **`aplicada`**, y `PLAN-45`, el cambio del lector desde la web, **`aplicada`**. Es la tercera puerta: sin plan aprobado no se escribe código |
| Specs ya aplicadas | `specs/aplicadas/` | Las que terminaron en `estado: aplicada`, con su `commit_de_aplicacion` en el frontmatter. Hoy `SPEC-03` referencias del dominio, `SPEC-04` puerta de capítulo y reclasificaciones, `SPEC-05` caducidad de las afirmaciones condicionales, `SPEC-06` estructura de `harness/` y `SPEC-07` modelo de fallo del pipeline y `SPEC-08` estados del trabajo y observabilidad `SPEC-10` huecos que la rama `main` ya sufrió `SPEC-11` observabilidad del pipeline, `SPEC-12` formas reducidas del recorte, `SPEC-13` durabilidad de los hechos, `SPEC-14` delegación en vez de API, `SPEC-15` lo que declara el plan, `SPEC-16` revelar es aprender, `SPEC-17` el conocimiento y su instante, `SPEC-18` el POV que nadie declara, `SPEC-19` lo prometido y lo entregado, `SPEC-20` qué se puede editar a mano `SPEC-25` el destinatario, la entrevista y las palabras vetadas, que abre el examen de la novela para regalar y lleva en su anexo la plantilla de la ficha, y `SPEC-26` el pipeline de la novela regalo (planificador con revisor, editor, validadores de personalización y los dos hooks), `SPEC-34` la pseudonimización de los nombres (`F-146`: los nombres se escriben en un campo propio y los agentes ven nombres inventados), `SPEC-35` las seis pantallas rediseñadas y la generación de noche (Xime y su cuaderno), `SPEC-36` la estantería de madera, el escritor en su mesa y la administración sin login, `SPEC-37` la historia de cada novela en la administración, `SPEC-38` la matriz por capítulo, con la línea de tiempo como segunda pestaña, `SPEC-39` publicar y reanudar desde la web, `SPEC-40` el protagonista ante los agentes, `SPEC-41` retirar de la estantería y el techo entre capítulos `SPEC-42` la administración enseña la estantería, las mismas y en el mismo orden, `SPEC-43` la lectura es para el lector y el estado de cada escena va a la administración, `SPEC-44` generar o reanudar desde la administración, `SPEC-45` el cambio del lector desde la web, `SPEC-32` la dedicatoria en `Obra` y la extensión preguntada, y `SPEC-30` la puerta de publicación con Lean, `SPEC-27` la exportación a PDF y `SPEC-28` las tools de lectura de la story bible. **`SPEC-02`, vocabularios, no está**: se aplicó y su fichero se retiró antes de existir esta carpeta, y no se puede recuperar porque nunca llegó a commitearse. Es la única excepción y no se repite |
| Código del backend | `backend/` | Servicio FastAPI con `app/commons/` y `app/features/`. Features: `escaleta`, `generacion`, `verificacion`, `consolidacion`, `contexto`, `orquestacion`, `auditoria`, `recuperacion`, `edicion`, `brief`, `cronologia` (dónde se usa cada hecho y la cronología de la fábula: `SPEC-21`), `entrevista` (la ficha del destinatario por turnos y el texto libre: `SPEC-25`) y `politica` (las palabras vetadas en tres niveles: `SPEC-25`); y en `auditoria/` la puerta de publicación con Lean (`SPEC-30`), que necesita `lake`. Lo que comparten las dos últimas —el detector de vetadas y el audit log— está en `app/commons/politica/`, y los modelos de la ficha en `app/commons/dominio/destinatario.py`. La CLI de la entrevista es `backend/entrevista_cli.py`. `regalo` lee lo que la web de la novela regalo enseña -la estantería, el gasto y la generación en vivo- y el lanzamiento desde la web vive en `orquestacion/router_regalo.py` (`SPEC-33`); `planificacion` lleva de la ficha a un plan aprobado (`SPEC-26`); los dos hooks de Claude Code están en `backend/hooks/`, el servidor MCP de la story bible (`SPEC-28`) en `backend/herramientas/`, la novela regalo con agentes reales se lanza con `backend/novela_regalo.py`, que gasta dinero, un brief de evaluación con `backend/evaluar.py` (`SPEC-31`), que también gasta, y un cambio del lector sobre una novela ya escrita con `backend/pedir_cambio.py` (la cascada de `SPEC-23`), que gasta solo con `--confirmo-el-gasto`; `evaluacion` es su feature (briefs, libro de gasto, tabla y rastro de exfiltración). Las dependencias en `backend/requirements.txt`; las pruebas con `python -m pytest app -q` desde `backend/` |
| Presentación | `presentacion/` | Los anexos en PDF (`anexo-enunciado.pdf`, de `EXAMEN.md`; `anexo-roadmap-costes.pdf`, de `ROADMAP-COSTES.md`) y un `README.md` con el idioma (castellano) y lo que falta: el deck en PDF y editable, y el vídeo de demo. Los datos del deck, con su fuente, en `docs/presentacion/datos.md` |
| Levantar la web | `arrancar-web.ps1` | La API (8000) y la web (5173) en segundo plano, con `backend/web.db` por defecto. Sin `node` en el PATH usa el de VS Code; busca `claude` en el PATH o en la extensión de VS Code |
| Frontend y contrato | `frontend/`, `contrato/openapi.json` | La lectura web (`SPEC-22`, `PLAN-22`): Vite, React y TypeScript con FSD v2.1, probada sin backend contra fixtures que validan con el congelado. `contrato/openapi.json` se deriva del backend y lo compara `harness/documentos/contrato.py`. Las órdenes, en `CLAUDE.md` § Comandos |
...
- Lo que `EXAMEN.md` exige que el repositorio incluya, y no existe todavía (§ "Los repositorios deben incluir también"): el vídeo de demo en `presentacion/`. Ya existen: `ejemplos/novela-ejemplo.pdf`, `presentacion/` con sus anexos, `README.md` en la raíz, `backend/.env.example`, `ejemplos/brief-ejemplo.json`, `.claude/commands/`, la memoria en `.claude/memory/` y `.mcp.json` con Playwright MCP.
