...
| Specs en curso | `specs/` | Un fichero por spec. **`aprobada`**: `SPEC-01` backend del harness, `SPEC-21` usos de un hecho y cronología de la fábula, `SPEC-22` frontend y contrato congelado, `SPEC-23` qué es regenerar en una obra acumulativa (con la salida decidida por el arrastre medido), `SPEC-24` dónde se declara una analepsis, y las tres que salen de `EXAMEN.md` (`docs/cobertura-examen.md`): `SPEC-29` Langfuse y qué sube y qué no y `SPEC-31` la evaluación, y `SPEC-33` la novela regalo en la web (estantería, entrevista, generación en vivo y coste). **`en_revision`**: `SPEC-09` versión de `Resumen` y `SPEC-34` la pseudonimización de los nombres (`F-146`) |
