...
def guardar_resumen(con, escena, orden, texto, hechos_clave=None, nivel="escena"):
    ...
    asegurar_tablas(con)
    with con:
        con.execute("INSERT OR REPLACE INTO resumen VALUES (?, ?, ?, ?, ?, ?)",
                    (escena, orden, escena, nivel, texto, json.dumps(hechos_clave)))


def resumenes_hasta(con, orden_actual):
    """Los resumenes de las escenas anteriores, en orden narrativo.
...
def actualizar_fichas(con, escena, orden, entidades):
    ...
    with con:
        for entidad, resumen in entidades.items():
            con.execute("INSERT OR REPLACE INTO ficha VALUES (?, ?, ?, ?)",
                        (entidad, orden, escena, resumen))


def fichas_en(con, orden_actual):
