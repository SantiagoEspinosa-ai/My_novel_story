...
| VER-64 | **Con qué frecuencia `INV-03` bloquea una generación larga** | Sigue sin dato, y la ejecución de seis escenas **no lo dio**: bloqueó cero veces porque no hubo una sola revelación que comprobar (`F-29`, `F-30`). Hace falta repetirla con los `HechoCanonico` declarados. **La parada que esperábamos no ocurrió, y eso no es una buena noticia: es que la puerta no llegó a mirar** | Ninguna. Nace aquí |
| VER-37 | El reparto por niveles de `CLAUDE.md` basta para una escena real | **Medido tres veces, y las tres el contexto se quedó corto.** En la generación de seis escenas de `PLAN-01` F6: **1.300 tokens de contexto real** frente al techo de 100.000, y **ni un recorte**. Crece —las condensaciones fueron de 0 a 274— pero por dos órdenes de magnitud por debajo. Y ahora se sabe **por qué**: las fichas salieron vacías y el registro de conocimiento no creció, por el punto muerto de `F-29`. **La dependencia no era una obra larga: era que el material existiera.** Se cierra cuando `F-29` esté arreglado y una generación vuelva a medirse | Ninguna. Nace aquí |
