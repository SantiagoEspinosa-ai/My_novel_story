...
| F-2 | El "Dónde vive" de `VER-01` y `VER-38` apuntaba a `harness/esquema/`, que no es ninguna de las carpetas de la estructura del harness | **Cerrado a medias.** `VER-01` y `VER-38` pasan a `commons/dominio/tests/` y `commons/invariantes/tests/`, y `VER-22` a `CI`, porque es *mutation testing* sobre casos negativos que viven repartidos por las features. Al reubicarlas salió que **no existe ninguna estructura declarada del harness**: se borró con la carpeta. Las seis filas que quedan —`VER-45`, `VER-46`, `VER-26`, `VER-40`, `VER-51`, `VER-30`— no son tests de una feature sino un linter de documentos y evals contra un modelo, y darles casa es declarar esa estructura, que es una decisión de arquitectura. Pendiente de `SPEC-06` |
