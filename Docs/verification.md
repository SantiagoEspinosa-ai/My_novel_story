...
| F-58 | **El Escritor nunca ha visto el contexto que el harness le monta: recibe los tamaños de los bloques, no su texto.** `obra.generar_obra` monta los bloques (`ensamblado.montar`), calcula sus tokens (`ensamblado.tamanos`) y pasa **los tamaños** a `ciclo.ejecutar`; `bucle._prompt` los vuelca en «LO QUE YA ES VERDAD EN LA FICCION» como `[["inmutable", 66], ["escena_anterior", 0], ...]`. Así que el bloque inmutable (premisa, estilo, género y tono), la escena anterior, los resúmenes y las fichas **no llegan al modelo**. Y la sinopsis de cada capítulo tampoco: `guardar_escaleta` no la guarda y el objetivo es solo `{eje, signo}`. Aparece desde `9089f52`. Consecuencias: la obra de diez capítulos se escribió sin premisa ni memoria, y **el contexto «medido» de 1.339 tokens que cita `CLAUDE.md` mide lo que se monta, no lo que se envía**: sesga hacia abajo y hacia la conclusión cómoda de que sobra presupuesto. No falló nada porque ninguna prueba miraba el contenido del prompt, solo que el recorte cuadrara | **Abierto, sin arreglar a propósito**: el usuario pide ver el diagnóstico antes de aplicar un arreglo. Lo delata `test_escribir_encadena_plan_montaje_y_generacion`, marcada como fallo esperado estricto: se pondrá en rojo el día que se arregle. Encontrado en `PLAN-26`, al encadenar la novela regalo |
