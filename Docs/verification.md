...
| VER-64 | **Con qué frecuencia `INV-03` bloquea una generación larga** | No hay ni una medida. Se sabe que bloquea —pasó en la segunda ejecución, con `per-ana` revelando lo que solo sabía su hermana— y no se sabe **cada cuánto**. **Si la generación de la Fase F se para en la tercera escena, eso no es una incidencia: es el dato.** Hace falta antes de intentar sesenta escenas, porque decide si el cuello de botella es tolerable o si hay que trabajar el prompt | Ninguna. Nace aquí |
| VER-37 | El reparto por niveles de `CLAUDE.md` basta para una escena real | **No es falta de medida: es una dependencia.** Se midió dos veces y las dos el contexto se quedó corto —138 y 9.237 tokens de 100.000, sin un solo recorte—, porque **un fixture escrito a mano no llega al techo**. Hace falta el estado **acumulado** de muchas escenas: fichas que crecen, resúmenes que se apilan, un registro de conocimiento con historia. **Se cierra con una generación larga, no con una ejecución más** | Ninguna. Nace aquí |
